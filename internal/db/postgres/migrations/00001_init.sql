-- +goose Up
-- Baseline schema for the PostgreSQL backend. Idempotent (CREATE ... IF NOT
-- EXISTS) so databases created before goose was adopted are brought under
-- version tracking without re-running their DDL. Same LCD types as the other
-- engines (TEXT + BIGINT, `gone` as BIGINT 0/1) so the sqlc packages generate
-- identical Go signatures.
