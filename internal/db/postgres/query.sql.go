...
SELECT id, name, description, source_ids, added_at FROM collections WHERE id = $1
`
...
SELECT id, name, config_json, added_at FROM configs WHERE id = $1
`
...
SELECT id, source_id, label, image_key, url, sample, title, status, gone, added_at, last_seen, decided_at FROM items WHERE id = $1
`
...
SELECT id, name, description, url, config_file, config_json, added_at, last_poll, last_added, last_error, config_id FROM sources WHERE id = $1
`
